...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print (help_msg)
        sys.exit(1)
    p_name = 'EthSyncBlockd' #守护进程名称
    pid_fn = '/tmp/EthSyncBlockd.pid' #守护进程pid文件的绝对路径
    log_fn = '/tmp/EthSyncBlockd.log' #守护进程日志文件的绝对路径
    err_fn = '/tmp/EthSyncBlockd.err.log' #守护进程启动过程中的错误日志,内部出错能从这里看到
